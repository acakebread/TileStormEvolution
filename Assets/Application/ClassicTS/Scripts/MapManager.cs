...
namespace GamePreviewNamespace
{
	public class MapManager : MonoBehaviour
	{
		...
		public bool IsScrambled => isScrambled;
		...
		public void Initialize(string mapName)
		{
			...
			for (int index = 0; index < tileMap.Length; index++)
			{
				...
				var coord = GetTileCoordinates(index);
				GameObject tileObj = new GameObject($"{tiles[index].Properties.Type}_{coord.X}_{coord.Z}");
		...
		public void Scramble()
		{
			...
			for (int index = 0; index < tiles.Length; index++)
			{
				...
				if (scrambledIndex >= 0 && scrambledIndex < tiles.Length)
					scrambledTiles[scrambledIndex] = tiles[index];
			}
		...
		{
			if (currentMap?.mixed?.TileData?.unpacked_bytes == null || tiles == null || !isScrambled)
			{
				Debug.LogWarning("Cannot unscramble: invalid map, tiles, or not scrambled");
				return;
			}

			var unscrambledTiles = new TileData[tiles.Length];
			var offsets = currentMap.mixed.TileData.unpacked_bytes;
			for (int index = 0; index < tiles.Length; index++)
			{
				int scrambledIndex = index + offsets[index];
				if (scrambledIndex >= 0 && scrambledIndex < tiles.Length)
					unscrambledTiles[index] = tiles[scrambledIndex];
			}

			tiles = unscrambledTiles;
			isScrambled = false;
			UpdateTileObjectNamesAndPositions();
		}
