...
namespace ClassicTilestorm
{
	internal static class TileRouteAssembler
	{
		...
		private const bool UseDistanceHeuristic = true;
		private const bool AllowReverseSourceCandidates = false;
		...
		private readonly struct Anchor
		{
			...
			public readonly int RootDirection;

			public Anchor(int staticCell, int playableCell, int direction, int rootDirection = 0)
			{
				StaticCell = staticCell;
				PlayableCell = playableCell;
				Direction = direction;
				RootDirection = rootDirection != 0 ? rootDirection : direction;
			}
		}
		...
		private sealed class IslandContext
		{
			...
			public IslandContext(int[] baseGrid, bool[] rawPlayable, string islandKey,
				Dictionary<int, Queue<int>> navTiles, int navBudget, int reusableCrossroads,
				int rawCount, List<IslandRouteConstraint> existingConstraints)
			{
				BaseGrid = baseGrid;
		...
		public static bool TryAssemble(Map map, int sourceTile, int destinationTile, int previousTile, out Result result)
		{
			...
			{
				if (TrySolveIslandChain(state, sourceAnchor, context,
					new Dictionary<string, HashSet<string>>(),
					new Dictionary<string, List<IslandRouteConstraint>>(),
					0, out var assembledState, out var summary))
				{
					result = new Result { State = assembledState, Summary = summary };
					return true;
				}
				lastSummary = summary;
			}
		...
		private static bool TrySolveIslandChain(
			...
			Dictionary<string, HashSet<string>> consumedAnchors,
			...
			out string summary)
		{
			...
			if (IsAnchorConsumed(consumedAnchors, islandKey, sourceAnchor))
			{
				summary = $"island {islandKey} source anchor already consumed.";
				return false;
			}

			FilterReachableExits(baseGrid, rawPlayable, destinationAnchors, context.DestinationTile, context.Width, context.Height);
			RemoveConsumedAnchors(destinationAnchors, consumedAnchors, islandKey);

			var navTiles = CollectMovableNavTiles(state, rawPlayable, context.Rules);
			var island = new IslandContext(
				baseGrid, rawPlayable, islandKey, navTiles,
				CountNavTiles(navTiles), CountReusableCrossroads(navTiles),
				Count(rawPlayable), GetIslandConstraints(islandConstraints, islandKey));

			foreach (var destinationAnchor in destinationAnchors)
			{
				context.Attempts++;

				if (!TrySolveSingleIsland(state, sourceAnchor, destinationAnchor, island, context, out var islandState, out var islandSummary))
				{
				...
				var nextConsumed = CloneConsumedAnchors(consumedAnchors);
				AddConsumedAnchor(nextConsumed, islandKey, sourceAnchor);
				AddConsumedAnchor(nextConsumed, islandKey, destinationAnchor);

				var nextConstraints = CloneIslandConstraints(islandConstraints);
				AddIslandConstraint(nextConstraints, islandKey, new IslandRouteConstraint(sourceAnchor, destinationAnchor));

				if (TrySolveIslandChain(islandState, continuation.NextIslandAnchor, context, nextConsumed, nextConstraints, depth + 1, out assembledState, out var chainSummary))
				{
			...
			if (destinationAnchors.Count == 0)
				summary = $"source {sourceAnchor.StaticCell}->{sourceAnchor.PlayableCell}, area {island.RawCount}, no static-nav exit.";
			...
			out string summary)
		{
			...
			if (ValidateIslandConstraints(state, constraints, context.Rules, context.Width, context.Height))
			{
				...
				summary = $"island already navigable; area {island.RawCount}";
				return true;
			}

			if (IsFullyOccupiedByNavTiles(island))
			{
				summary = "island fully occupied by nav tiles";
				return false;
			}

			var solutionArea = CullSolutionArea(island.BaseGrid, island.RawPlayable,
				sourceAnchor.PlayableCell, destinationAnchor.PlayableCell,
				island.NavBudget, island.ReusableCrossroads, context.Width, context.Height);
			...
			if (!TryExtendRoute(searchGrid, sourceAnchor.PlayableCell, sourceAnchor.Direction,
				destinationAnchor.PlayableCell, Navigation.GetOppositeDirection(destinationAnchor.Direction),
				solutionArea, counts, visited, navVisits, route,
				context.Width, context.Height, ref nodes, ref searchBudgetExhausted))
			{
				var reason = searchBudgetExhausted ? $"search budget exhausted at {nodes}/{NodeBudget} nodes" : "no compatible chain";
				summary = $"no route found, area {island.RawCount}->{Count(solutionArea)}, {reason}.";
				return false;
			}

			if (!TryBuildState(state, island.RawPlayable, route, island.NavTiles, out assembledState))
			{
				summary = "found route but failed to build state.";
				return false;
			}
			...
			if (!ValidateIslandConstraints(assembledState, constraints, context.Rules, context.Width, context.Height))
			{
				...
				{
					summary = $"assembled route with {route.Count} tile(s), repaired by swapping.";
					return true;
				}
				summary = "assembled route but constraints could not be preserved.";
				return false;
			}

			summary = $"assembled route with {route.Count} movable tile(s).";
			return true;
		}
		...
		private static int[] BuildRules(Map map)
		{
			...
			for (var i = 0; i < rules.Length; i++)
			{
				...
				var variant = variantIndex >= 0 && map.variants != null && variantIndex < map.variants.Length
					? map.variants[variantIndex] : default;

				var definition = ResourceManager.ResolveDefinition(variant.hash, out _);
				var baseFlags = ((IFlagAccess)definition)?.Flags ?? 0;
				var rotatedNav = Navigation.Rotate(baseFlags & (int)DefinitionFlags.DirMask, Mathf.RoundToInt(variant.angle));
		...
		private static int[] BuildScratchGrid(int[] state, int[] rules)
		{
			...
			{
				if (!TryGetRule(state, i, rules, out var rule)) continue;
				grid[i] = CreateCellBits(IsStatic(rule), IsStatic(rule) ? GetNav(rule) : 0);
			}
			return grid;
		}

		private static int[] BuildLiveNavGrid(int[] state, int[] rules)
		{
			var grid = new int[state.Length];
			for (var i = 0; i < state.Length; i++)
			{
				if (!TryGetRule(state, i, rules, out var rule)) continue;
				grid[i] = CreateCellBits(IsStatic(rule), GetNav(rule));
			}
			return grid;
		}

		private static List<Anchor> FindAnchors(int[] grid, int origin, bool[] targetPlayableArea, int width, int height)
		{
			var anchors = new List<Anchor>();
			if (origin < 0 || origin >= grid.Length) return anchors;
			if (!IsStaticNavAt(grid, origin)) return anchors;

			var queue = new Queue<(int cell, int rootDirection)>();
			...
			{
				var (current, rootDirection) = queue.Dequeue();
				var currentNav = GetNav(grid[current]);

				foreach (var direction in Navigation.Directions)
				{
					if ((currentNav & direction) == 0) continue;
					var branchRoot = rootDirection != 0 ? rootDirection : direction;

					var next = Navigation.GetAdjacentTile(current, direction, width, height);
					if (next < 0) continue;

					if (IsPlayableAt(grid, next))
					{
						if (targetPlayableArea == null || targetPlayableArea[next])
							AddAnchor(anchors, new Anchor(current, next, direction, branchRoot));
						continue;
					}

					if (HasStaticConnection(grid, current, next, direction) && visited.Add(next))
						queue.Enqueue((next, branchRoot));
				}
			}
			return anchors;
		}

		private static void FilterReverseSourceAnchors(int[] liveGrid, List<Anchor> anchors, int sourceTile, int previousTile, int width, int height)
		{
			if (AllowReverseSourceCandidates || anchors == null || anchors.Count == 0 || previousTile < 0)
				return;

			var reverseRootDirection = FindRootDirectionToTile(liveGrid, sourceTile, previousTile, width, height);
			if (reverseRootDirection == 0)
				return;

			anchors.RemoveAll(anchor => anchor.RootDirection == reverseRootDirection);
		}

		private static int FindRootDirectionToTile(int[] grid, int sourceTile, int targetTile, int width, int height)
		{
			if (sourceTile < 0 || targetTile < 0 || sourceTile >= grid.Length || targetTile >= grid.Length)
				return 0;
			...
			while (queue.Count > 0 && visited.Count < NodeBudget)
			{
				...
				foreach (var direction in Navigation.Directions)
				{
					...
					var next = Navigation.GetAdjacentTile(current, direction, width, height);
					if (next < 0 || GetNav(grid[next]) == 0)
						continue;

					if ((GetNav(grid[next]) & Navigation.GetOppositeDirection(direction)) == 0 || !visited.Add(next))
						continue;

					queue.Enqueue((next, rootDirection != 0 ? rootDirection : direction));
				}
			}
		...
		private static bool[] FloodPlayableArea(int[] grid, int source, Anchor ignoredEntrance, List<Anchor> exits, int width, int height)
		{
			...
			while (queue.Count > 0)
			{
				...
				foreach (var direction in Navigation.Directions)
				{
					...
					if (!IsStaticNavAt(grid, next)) continue;

					var staticToPlayableDirection = Navigation.GetOppositeDirection(direction);
					if ((GetNav(grid[next]) & staticToPlayableDirection) == 0) continue;

					var exit = new Anchor(next, current, staticToPlayableDirection);
					if (exit.StaticCell == ignoredEntrance.StaticCell &&
						exit.PlayableCell == ignoredEntrance.PlayableCell &&
						exit.Direction == ignoredEntrance.Direction) continue;
		...
		{
			if (grid == null || playableArea == null || exits == null) return;
		...
		private static bool TryTraceExitContinuation(int[] grid, bool[] playableArea, Anchor exit, int destinationTile, int width, int height, out ExitContinuation continuation)
		{
			...
			var queue = new Queue<(int cell, int incoming)>();
			var visited = new HashSet<int>();
			var initialIncoming = Navigation.GetOppositeDirection(exit.Direction);
			queue.Enqueue((exit.StaticCell, initialIncoming));
			...
			while (queue.Count > 0 && visited.Count < NodeBudget)
			{
				...
				var next = Navigation.GetAdjacentTile(current, outgoing, width, height);
		...
		{
			if (playableArea == null) return "<null>";
			var cells = new List<int>();
			for (var i = 0; i < playableArea.Length; i++)
				if (playableArea[i]) cells.Add(i);
		...
		private static Dictionary<string, HashSet<string>> CloneConsumedAnchors(Dictionary<string, HashSet<string>> consumedAnchors)
		{
			...
			if (consumedAnchors == null) return clone;
		...
		{
			if (consumedAnchors == null || string.IsNullOrEmpty(islandKey)) return;
		...
		private static bool IsAnchorConsumed(Dictionary<string, HashSet<string>> consumedAnchors, string islandKey, Anchor anchor)
			=> consumedAnchors != null && consumedAnchors.TryGetValue(islandKey, out var anchors) && anchors.Contains(BuildAnchorKey(anchor));
		...
		{
			if (anchors == null || consumedAnchors == null || !consumedAnchors.TryGetValue(islandKey, out var consumed)) return;
		...
		private static string BuildAnchorKey(Anchor anchor) => $"{anchor.StaticCell}:{anchor.PlayableCell}:{anchor.Direction}";
		...
		private static Dictionary<string, List<IslandRouteConstraint>> CloneIslandConstraints(Dictionary<string, List<IslandRouteConstraint>> constraints)
		{
			...
			if (constraints == null) return clone;
		...
		{
			if (constraints == null || string.IsNullOrEmpty(islandKey)) return;
			if (!constraints.TryGetValue(islandKey, out var list))
			{
				list = new List<IslandRouteConstraint>();
				constraints[islandKey] = list;
			}
			list.Add(constraint);
		}
		...
		{
			var constraints = existingConstraints != null ? new List<IslandRouteConstraint>(existingConstraints) : new List<IslandRouteConstraint>();
			constraints.Add(new IslandRouteConstraint(sourceAnchor, destinationAnchor));
			return constraints;
		}

		private static bool TryRepairIslandBySwapping(int[] state, bool[] islandCells, List<IslandRouteConstraint> constraints, int[] rules, int width, int height, out int[] repairedState)
		{
			repairedState = null;
			if (state == null || islandCells == null || constraints == null) return false;

			var cells = new List<int>();
			for (var i = 0; i < islandCells.Length; i++)
				if (islandCells[i]) cells.Add(i);
			...
			for (var a = 0; a < cells.Count; a++)
				for (var b = a + 1; b < cells.Count; b++)
				{
					...
					if (firstNav == secondNav) continue;
		...
		{
			if (state == null || rules == null) return false;
		...
		private static Dictionary<int, Queue<int>> CollectMovableNavTiles(int[] state, bool[] rawPlayable, int[] rules)
		{
			...
			{
				if (!rawPlayable[i]) continue;
				var logicalIndex = state[i];
				if (logicalIndex < 0 || logicalIndex >= rules.Length) continue;
		...
		private static bool[] CullSolutionArea(int[] grid, bool[] rawPlayable, int sourceEntry, int destinationEntry, int navBudget, int reusableCrossroads, int width, int height)
		{
			var solutionArea = (bool[])rawPlayable.Clone();
			if (navBudget <= 0) return solutionArea;
			...
			while (previousCount != Count(solutionArea))
			{
				...
				{
					if (!solutionArea[i]) continue;
					var reachable = sourceDistance[i] >= 0 && destinationDistance[i] >= 0;
					var routeLength = reachable ? AdjustRouteLengthForReusableCrossroads(sourceDistance[i], destinationDistance[i], reusableCrossroads) : int.MaxValue;
					if (reachable && routeLength <= navBudget) continue;
		...
		private static int AdjustRouteLengthForReusableCrossroads(int sourceDistance, int destinationDistance, int reusableCrossroads)
		{
			...
			if (reusableCrossroads <= 0 || sourceDistance <= 0 || destinationDistance <= 0) return routeLength;
		...
		private static int[] BuildDistanceMap(int[] grid, bool[] solutionArea, int source, int width, int height)
		{
			...
			if (source < 0 || source >= solutionArea.Length || !solutionArea[source]) return distance;
			...
			while (queue.Count > 0)
			{
				...
				{
					var next = Navigation.GetAdjacentTile(current, direction, width, height);
					if (next < 0 || !TryGetFillStepCost(grid, solutionArea, current, next, direction, out var cost)) continue;

					var nextDistance = distance[current] + cost;
					if (distance[next] >= 0 && distance[next] <= nextDistance) continue;

					distance[next] = nextDistance;
					if (cost == 0) queue.AddFirst(next);
		...
		private static bool TryExtendRoute(int[] grid, int current, int incomingDirection, int destinationCell, int exitDirection,
			bool[] solutionArea, Dictionary<int, int> navCounts, bool[] visited, HashSet<int> navVisits,
			List<RouteCell> route, int width, int height, ref int nodes, ref bool searchBudgetExhausted)
		{
			if (current < 0 || current >= solutionArea.Length) return false;

			if (GetNav(grid[current]) != 0)
			{
				var visitKey = current * 16 + incomingDirection;
				if (!navVisits.Add(visitKey)) return false;

				var outgoing = Navigation.CalculateNav(incomingDirection, GetNav(grid[current]));
				if (outgoing == 0)
				{
				...
				if (current == destinationCell && (outgoing & exitDirection) != 0) return true;

				foreach (var direction in OrderedOutputDirections(outgoing, current, destinationCell, width, height))
				{
					if (TryExtendRoute(grid, Navigation.GetAdjacentTile(current, direction, width, height), direction, destinationCell, exitDirection, solutionArea, navCounts, visited, navVisits, route, width, height, ref nodes, ref searchBudgetExhausted))
						return true;
				}
			...
			if (!solutionArea[current] || visited[current]) return false;

			foreach (var nav in OrderedAvailableNavMasks(navCounts, incomingDirection, current, destinationCell, width, height))
			{
				var outgoing = Navigation.CalculateNav(incomingDirection, nav);
				if (outgoing == 0) continue;
				...
				{
					var next = Navigation.GetAdjacentTile(current, direction, width, height);
					if (!CanAcceptIncoming(grid, next, direction, destinationCell, exitDirection, solutionArea, navCounts)) continue;
		...
		private static bool CanAcceptIncoming(int[] grid, int cell, int incomingDirection, int destinationCell, int exitDirection, bool[] solutionArea, Dictionary<int, int> navCounts)
		{
			if (cell < 0 || cell >= solutionArea.Length) return false;

			if (GetNav(grid[cell]) != 0)
			{
				var outgoing = Navigation.CalculateNav(incomingDirection, GetNav(grid[cell]));
				return outgoing != 0;
			}

			if (!solutionArea[cell]) return false;

			foreach (var pair in navCounts)
			{
				if (pair.Value <= 0) continue;
				var outgoing = Navigation.CalculateNav(incomingDirection, pair.Key);
				if (outgoing == 0) continue;
				return true;
			}
			return false;
		...
		private static bool TryBuildState(int[] state, bool[] rawPlayable, List<RouteCell> route, Dictionary<int, Queue<int>> navTiles, out int[] assembledState)
		{
			...
			for (var i = 0; i < rawPlayable.Length; i++)
			{
				if (!rawPlayable[i] || used.Contains(state[i])) continue;
				remaining.Enqueue(state[i]);
			}

			for (var i = 0; i < rawPlayable.Length; i++)
			{
				if (!rawPlayable[i] || routeCells.Contains(i) || remaining.Count == 0) continue;
		...
		private static List<int> OrderedAvailableNavMasks(Dictionary<int, int> navCounts, int incomingDirection, int current, int destinationCell, int width, int height)
		{
			...
			if (UseDistanceHeuristic)
			{
				masks.Sort((a, b) =>
				{
					var distA = BestOutputDistance(current, Navigation.CalculateNav(incomingDirection, a), destinationCell, width, height);
					var distB = BestOutputDistance(current, Navigation.CalculateNav(incomingDirection, b), destinationCell, width, height);
		...
		private static List<int> OrderedOutputDirections(int directions, int current, int destinationCell, int width, int height)
		{
			...
			foreach (var direction in Navigation.Directions)
			{
				if ((directions & direction) != 0) result.Add(direction);
			}
			...
			if (UseDistanceHeuristic)
			{
				result.Sort((a, b) =>
				{
					var nextA = Navigation.GetAdjacentTile(current, a, width, height);
					var nextB = Navigation.GetAdjacentTile(current, b, width, height);
					var distA = nextA < 0 ? int.MaxValue : Navigation.ManhattanDistance(nextA, destinationCell, width);
					var distB = nextB < 0 ? int.MaxValue : Navigation.ManhattanDistance(nextB, destinationCell, width);
					return distA.CompareTo(distB);
				});
			}
			return result;
		...
		private static int BestOutputDistance(int current, int directions, int destinationCell, int width, int height)
		{
			...
			foreach (var direction in Navigation.Directions)
			{
				...
				var next = Navigation.GetAdjacentTile(current, direction, width, height);
				if (next >= 0) best = Math.Min(best, Navigation.ManhattanDistance(next, destinationCell, width));
			}
			return best;
		}
		...
		{
			if (sourceTile == destinationTile || sourceTile < 0 || destinationTile < 0) return false;

			var sourceNav = GetNav(GetRuleAt(state, sourceTile, rules));
			foreach (var direction in Navigation.Directions)
			{
				if ((sourceNav & direction) == 0) continue;
				var visited = new HashSet<int>();
		...
		private static bool TryFollowFlexibleRoute(int[] state, int current, int direction, int destinationTile, int[] rules, int width, int height, HashSet<int> visited)
		{
			...
			var nextNav = GetNav(GetRuleAt(state, next, rules));
			if ((nextNav & Navigation.GetOppositeDirection(direction)) == 0) return false;
			if (next == destinationTile) return true;

			var key = next * 16 + direction;
			if (!visited.Add(key)) return false;

			var outgoing = Navigation.CalculateNav(direction, nextNav);
			foreach (var nextDirection in Navigation.Directions)
			{
				if ((outgoing & nextDirection) == 0) continue;
		...
		private static int CreateCellBits(bool isStatic, int nav) => (isStatic ? StaticMask : 0) | (nav & NavMask);
		private static bool IsStatic(int bits) => (bits & StaticMask) != 0;
		private static int GetNav(int bits) => bits & NavMask;
		private static int WithNav(int bits, int nav) => (bits & ~NavMask) | (nav & NavMask);

		private static bool IsPlayableAt(int[] grid, int cell) => grid != null && cell >= 0 && cell < grid.Length && !IsStatic(grid[cell]);
		private static bool IsStaticNavAt(int[] grid, int cell) => grid != null && cell >= 0 && cell < grid.Length && IsStatic(grid[cell]) && GetNav(grid[cell]) != 0;

		private static bool HasStaticConnection(int[] grid, int current, int next, int direction)
		{
			if (!IsStaticNavAt(grid, current) || !IsStaticNavAt(grid, next)) return false;
		...
		{
			rule = default;
			if (state == null || rules == null || cell < 0 || cell >= state.Length) return false;
			var logical = state[cell];
			if (logical < 0 || logical >= rules.Length) return false;
			rule = rules[logical];
			return true;
		}
		...
		{
			var count = 0;
			foreach (var pair in navTiles) count += pair.Value.Count;
			return count;
		}
		...
		private static int Count(bool[] area)
		{
			...
			var count = 0;
			for (var i = 0; i < area.Length; i++) if (area[i]) count++;
			return count;
		}
