...
namespace ClassicTilestorm
{
	public class TileColliderInfo : MonoBehaviour
	{
		...
		[HideInInspector] public int LogicalIndexOverride = -1;
	}
		...
		public bool TryGetHitTile(Camera camera, Vector3 screenPos, out int logicalIndex, out int visualIndex)
		{
			...
			if (Physics.Raycast(ray, out RaycastHit hit, 1000f))
			{
				...
				if (info != null && info.Map == this)
				{
					...
					logicalIndex = info.LogicalIndexOverride >= 0
						? info.LogicalIndexOverride
						: state != null && visualIndex >= 0 && visualIndex < state.Length
						? state[visualIndex]
						: visualIndex;
					return true;
				}
