...
Shader "Unlit/URPFrost"
{
    Properties
    {
        _BaseColor ("Base Color", Color) = (0.25, 0.25, 0.25, 1)
        _Depth ("Depth", Range(0, 1)) = 1.0 // Normalized depth
        _DepthMax ("Depth Max", Range(1, 512)) = 512 // Max iterations
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Define 4 sample offsets at 22.5-degree increments
                ...
                // Sample loop
                float ct = _Depth * _DepthMax; // Total iterations
                for (float d = 1; d <= ct; ++d)
                {
                    float2 texelOffset = _MainTex_TexelSize.xy * d * scale; // Perspective-correct sampling
                    for (int i = 0; i < 4; i++)
                    {
