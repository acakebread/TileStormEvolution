	...
	public class MeshDivisionTest : MonoBehaviour
	{
		...
		private void ApplySubdivision()
		{
			...
			if (enableSubdivision)
			{
				...
				subdividedMesh = MeshUtils.SubdivideMeshLongEdges(inputMesh, localAxis, maxSegmentLength, localCenter);
				...
				if (subdividedMesh != null && subdividedMesh.vertexCount > 0)
				{
					...
					if (useDoubleSidedShader && Debug.isDebugBuild)
					{
						...
						if (doubleSided != null)
						{
							...
							renderer.material.mainTexture = meshRenderer.material.mainTexture;
						}
						else
						{
					...
					if (Debug.isDebugBuild)
					{
						Debug.Log($"Created subdivided mesh: {subdividedMesh.vertexCount} vertices, {subdividedMesh.triangles.Length / 3} triangles");
					}
				}
				else
				{
