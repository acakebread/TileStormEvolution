...
namespace MassiveHadronLtd
{
	public static class ScreenSpaceUtil
	{
		...
		public static void OnGUI(IGridAtlas atlas, Rect rect, Vector2 coord = default)
		{
			...
			UpdateMeshesAndRTs(atlas, rect, coord, columns, rows, cellSize, coreW, coreH, padW, padH, invalid);
		...
		// ────────────────────────────────────────────────────────────────
		// New getters – update meshes/RTs but DO NOT draw to GUI
		// ────────────────────────────────────────────────────────────────
		public static RenderInfo GetGridRenderInfo(IGridAtlas atlas, Rect rect, Vector2 coord = default)
		{
			if (atlas == null || atlas.Texture == null) return default;
			...
			if (_gridRT == null) return default;

			var scaleW = (float)rect.width / coreW;
			var scaleH = (float)rect.height / coreH;
			var screenMarginW = (int)(marginW * scaleW);
			var screenMarginH = (int)(marginH * scaleH);

			var displayRect = new Rect(
				rect.x - screenMarginW,
				rect.y - screenMarginH,
				rect.width + screenMarginW * 2,
				rect.height + screenMarginH * 2
			);

			return new RenderInfo { Texture = _gridRT, ScreenRect = displayRect };
		}

		public static RenderInfo GetFocusRenderInfo(IGridAtlas atlas, Rect rect, Vector2 coord = default)
		{
			if (atlas == null || atlas.Texture == null) return default;
			...
			if (_focusRT == null || _gridMesh == null || invalid) return default;

			var lastBase = (columns * rows - 1) * 4;
			var verts = _gridMesh.vertices;

			if (lastBase + 3 >= verts.Length) return default;
			...
			if (quadWidth <= 0 || quadHeight <= 0) return default;
			...
			return new RenderInfo { Texture = _focusRT, ScreenRect = selRect };
		}

		// ────────────────────────────────────────────────────────────────
		// Shared update – called by OnGUI and getters (no drawing here)
		// ────────────────────────────────────────────────────────────────
		...
		private static void UpdateMeshesAndRTs(
			IGridAtlas atlas, Rect rect, Vector2 coord,
			int columns, int rows, int cellSize,
			int coreW, int coreH,
			int padW, int padH,
			bool invalid)
		{
			...
			var contentScaleX = (float)coreW / padW;
			var contentScaleY = (float)coreH / padH;
			var marginW_core = (padW - coreW) / 2;
			var marginH_core = (padH - coreH) / 2;

			var contentOffsetX = (float)marginW_core / padW;
			var contentOffsetY = (float)marginH_core / padH;
