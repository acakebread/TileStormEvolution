...
namespace MassiveHadronLtd
{
	...
	{
		// Main cache for the tinted cubemap
		private static Cubemap s_CurrentTintedCubemap;   // kept for backward compatibility in Compute methods
		private static Material s_CurrentSkyMat;

		// Ref-counted internal version (this is what actually manages lifetime)
		private static Ref<Cubemap> s_TintedRef;

		// Lightweight caches for compute functions
		...
		{
			// ... your existing SampleCubemap code (unchanged) ...
			dir.Normalize();
		...
		/// Only ONE tinted cubemap is kept in memory at any time.
		/// </summary>
		/// <summary>
		/// Returns a tinted version of the skybox as a Cubemap.
		/// Only ONE tinted cubemap is kept in memory at any time.
		...
		public static Cubemap GetTintedCubemap(Material skyMat, int resolution = 256)
		{
			...
			// Fast reuse
			if (skyMat == s_CurrentSkyMat && s_TintedRef?.Value != null)
				return s_TintedRef.Value;

			// Release previous tinted cubemap
			s_TintedRef?.Set(null);      // ← changed from .Assign
			s_TintedRef = null;

			InvalidateComputeCaches();

			// Create new ref-counted cubemap
			var tintedRef = Ref<Cubemap>.CreateCubemap(resolution, TextureFormat.RGBA32, false);
			var tintedCubemap = tintedRef.Value;

			tintedCubemap.filterMode = FilterMode.Trilinear;
			tintedCubemap.wrapMode = TextureWrapMode.Clamp;
			tintedCubemap.name = "TintedSkyReflection_" + (skyMat.name ?? "Unnamed");
