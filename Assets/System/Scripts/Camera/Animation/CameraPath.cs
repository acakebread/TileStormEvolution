...
namespace MassiveHadronLtd
{
	public class CameraPath : CameraBase
	{
		...
		public override void Awake()
		{
			...
			if (data.camera == null || delegates == null)
			{
			...
			var playerPosition = delegatesInstance.target.Invoke();
			var focusPoints = delegatesInstance.focusPoints?.Invoke();

			// Disable sequence by default
			...
			lastPlayerPos = nextPlayerPos = playerPosition;
			...
			var startFocusPoint = playerPosition;
			if (focusPoints != null && focusPoints.Count > 0)
			{
				var validFocusPoint = focusPoints.Where(p => Vector2.Distance(new Vector2(p.x, p.z), new Vector2(playerPosition.x, playerPosition.z)) >= MinFocusPointDistanceFromPlayer).ToList();
			...
			data.target = playerPosition + Vector3.up * VerticalOffset;
			...
			{
				Debug.DrawRay(data.lerpedTarget, Vector3.up * 5f, Color.red, DEBUG_DRAW_DURATION);
				Debug.DrawRay(data.target, Vector3.up * 5f, Color.blue, DEBUG_DRAW_DURATION);
			}
		}
		...
		public override void Update()
		{
			if (data.camera == null || delegates == null)
			{
				Debug.LogWarning("CameraPath.Update: Missing camera or delegates");
				return;
			}

			var delegatesInstance = delegates.Invoke();
			var playerTransformPosition = delegatesInstance.target.Invoke();
			...
			{
				var posDelta = playerTransformPosition - lastPlayerPos;
				nextPlayerPos = SmoothingUtils.SmoothVector(nextPlayerPos, playerTransformPosition + posDelta * 2f, ProjectionSmoothingRate, Time.deltaTime, CameraData.TargetFPS);

				var easedSequenceTimer = SmoothingUtils.Ease(sequenceDuration > 0 ? 1f - Mathf.Clamp01(sequenceTimer / sequenceDuration) : 1f);

				// Update target
				data.target = Vector3.Lerp(data.lerpedTarget, nextPlayerPos + Vector3.up * VerticalOffset, easedSequenceTimer);

				// Update Bezier P1 (camera path mid point) and P2 (camera path Dst) with player movement
				var playerDelta = playerTransformPosition - lastPlayerPos;
				...
				lastPlayerPos = playerTransformPosition;
			}

			// Update camera lerping
			data.smoothing = SmoothingUtils.Smooth(data.smoothing, 16, Time.deltaTime, CameraData.TargetFPS);
		...
		private (Vector3 src, Vector3 dst) SampleCameraPosition(Vector3 midPoint, Vector3 pathDir, Vector3 perpendicular, float lozengeMajor, float lozengeMinor)
		{
			...
			if (DEBUG_VISUALIZE_LOZENGE)
			{
				...
				var perimeterSrcV3 = new Vector3(perimeterPointSrc.x, 0f, perimeterPointSrc.y);
				var perimeterDstV3 = new Vector3(perimeterPointDst.x, 0f, perimeterPointDst.y);
				Debug.DrawRay(perimeterSrcV3, Vector3.up * 5f, Color.yellow, DEBUG_DRAW_DURATION);
				Debug.DrawRay(perimeterDstV3, Vector3.up * 5f, Color.yellow, DEBUG_DRAW_DURATION);
...
//					.Where(p => Vector2.Distance(new Vector2(p.x, p.z), new Vector2(startFocusPoint.x, startFocusPoint.z)) >= MinFocusPointDistanceFromPlayer)
//					.ToList();
//				if (validFocusPoint.Count > 0) startFocusPoint = validFocusPoint[Random.Range(0, validFocusPoint.Count)];
//			}
...
//				// Update Bezier P1 (camera path mid point) and P2 (camera path Dst) with player movement
//				var playerDelta = delegatesInstance.target.Invoke() - lastPlayerPos;
//				bezierData.P1 += playerDelta * 0.5f;
//				bezierData.P2 += playerDelta;
...
//			var u = 1f - t;
//			var tt = t * t;
//			return u * u * bezierData.P0 + 2f * u * t * bezierData.P1 + tt * bezierData.P2;
//		}

//		private (Vector3, Vector3) SampleCameraPosition(Vector3 midPoint, Vector3 pathDir, Vector3 perpendicular, float lozengeMajor, float lozengeMinor)
//		{
//			var midPointXZ = new Vector2(midPoint.x, midPoint.z);
//			var targetXZ = new Vector2(data.target.x, data.target.z);
//			var (src, _, _, _, _, _) = GeneratePointOutsideLozenge(midPointXZ, pathDir, perpendicular, lozengeMajor, lozengeMinor);
//			var (dst, _, _, _, _, _) = GeneratePointOutsideLozenge(midPointXZ, pathDir, perpendicular, lozengeMajor, lozengeMinor);
//			var srcXZ = new Vector2(src.x, src.z);
//			var dstXZ = new Vector2(dst.x, dst.z);
