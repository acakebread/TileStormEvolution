...
namespace MassiveHadronLtd
{
	public class CinemaCameraOrbit : CinemaCameraBase
	{
		...
		private const float SmoothingRate = 16f; private float orbitHeightSrc;
		...
		protected override void StartCinemaSequence(ref CameraData data)
		{
			...
			data.shake = 1f;
			data.targetSrc = data.targetDst = playerTransform.position + Vector3.up * VerticalOffset;
			...
			var minRadiusSrc = CalculateMinOrbitRadius(orbitHeightSrc, data.targetDst.y);
			var minRadiusDst = CalculateMinOrbitRadius(orbitHeightDst, data.targetDst.y);
			...
			data.originDst = data.originSrc = data.targetSrc + SampleOrbitPosition(orbitStartAngle, orbitEndAngle, 0f);
		...
		{
			//update target
			data.targetDst = predictedPlayerPosition + Vector3.up * VerticalOffset;

			//update camera dest position and FOV
			data.originDst = data.targetDst + SampleOrbitPosition(orbitStartAngle, orbitEndAngle, easedSequenceTimer);
			...
			data.smoothing = SmoothingUtils.Smooth(data.smoothing, SmoothingRate, currentSequenceDuration, Time.deltaTime, CameraData.TargetFPS);
		}
