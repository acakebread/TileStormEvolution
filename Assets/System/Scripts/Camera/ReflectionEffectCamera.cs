...
namespace MassiveHadronLtd
{
	...
	public class ReflectionEffectCamera : MonoBehaviour
	{
		...
		private void SafeReleaseAndNull(ref RenderTexture rt)
		{
			...
			if (mainCamera != null && mainCamera.targetTexture == rt) mainCamera.targetTexture = null;
			if (reflectionCamera != null && reflectionCamera.targetTexture == rt) reflectionCamera.targetTexture = null;
			if (textureCamera != null && textureCamera.targetTexture == rt) textureCamera.targetTexture = null;
		...
		public void CreateOrResizeReflectionTexture(int targetWidth, int targetHeight)
		{
			...
			// Output texture handling for render-to-texture mode
			if (isRenderToTextureMode)
			{
