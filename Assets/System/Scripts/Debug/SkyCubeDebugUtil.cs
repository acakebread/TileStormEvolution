	...
	public class SkyCubeDebugUtil : MonoBehaviour
	{
		...
		private void UpdateFromSkybox(Material skybox = null)
		{
			...
			brightColor = ImageProcessing.ComputeBrightColorWithHistogram(
				panoramaTexture.GetPixels(),        // Force Color[] by calling GetPixels()
				out lumHistogram,
				out maxLum,
				out brightPixelCount,
				threshold: 0.85f);

			// Find sun UV - use bottom half scan (as per your inspector tests)
			sunUV = ImageProcessing.FindSunUV(panoramaTexture, scanAboveHorizonOnly: true);
		}
		...
		private void OnGUI()
		{
			...
			GUILayout.BeginArea(new Rect(10, 10, 620, 920));
			GUILayout.BeginVertical(GUI.skin.box);

			GUILayout.Label("<size=18><b>Sky Cubemap Debug - Linear Panorama</b></size>");
			GUILayout.Space(10);
			...
			GUILayout.Space(10);
			GUILayout.Label("Linear Panorama Preview:");
			...
			if (null != panoramaTexture)
			{
				...
				// Horizontal line
				GUI.DrawTexture(new Rect(markerPos.x - 14, markerPos.y - 1, 28, 2), Texture2D.whiteTexture);
				// Vertical line
				GUI.DrawTexture(new Rect(markerPos.x - 1, markerPos.y - 14, 2, 28), Texture2D.whiteTexture);
			...
			Rect histRect = GUILayoutUtility.GetRect(580, 160);
			GUI.Box(histRect, "");

			if (lumHistogram.Count > 0)
			{
				float barWidth = histRect.width / lumHistogram.Count;
				for (int i = 0; i < lumHistogram.Count; i++)
				{
					float barHeight = lumHistogram[i] * histRect.height;
					Rect bar = new Rect(histRect.x + i * barWidth,
									   histRect.y + histRect.height - barHeight,
...
//			flippedTexture = panoramaTexture;//.FlipVertically();
...
//			{
//				GUI.DrawTexture(previewRect, flippedTexture, ScaleMode.StretchToFill);
