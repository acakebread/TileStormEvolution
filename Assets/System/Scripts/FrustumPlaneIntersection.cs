...
public static class FrustumPlaneIntersection
{
	...
	public static bool GenerateFrustumPlaneIntersectionMesh(Camera camera, Vector3 planeNormal, float planeOffset, Mesh targetMesh)
    {
        ...
        {
            if (AddIntersection(plane, nearCorners[i], farCorners[i], out var point)) points[pointCount++] = point;
            if (AddIntersection(plane, nearCorners[i], nearCorners[(i + 1) % 4], out point)) points[pointCount++] = point;
            if (AddIntersection(plane, farCorners[i], farCorners[(i + 1) % 4], out point)) points[pointCount++] = point;
        }
