		...
		foreach (var pair in data)
		{
			if (pair.Value is Dictionary<string, object> nestedDict)
			{
				foreach (var nestedPair in nestedDict)
				{
					...
					sb.Append($"                if (result._{SanitizeKey(pair.Key)} != null{nestedNullCheck}) Debug.Log($\"{pair.Key}.{nestedPair.Key}: {{result._{SanitizeKey(pair.Key)}._{SanitizeKey(nestedPair.Key)}}}\"); // {nestedPair.Value}\n");
				}
			}
			else if (pair.Value is object[] array)
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					if (array[i] is Dictionary<string, object> memberDict)
					{
						foreach (var key in allKeys)
						{
							if (memberDict.ContainsKey(key))
							{
								...
								{
									sb.Append($"                if (result._{SanitizeKey(pair.Key)}.Length > {i}{nullCheck}) Debug.Log($\"{pair.Key}[{i}].{key}: {{string.Join(\", \", result._{SanitizeKey(pair.Key)}[{i}]._{SanitizeKey(key)})}}\"); // {JsonTocs.ToJson(value)}\n");
								}
								else if (value is Dictionary<string, object> detailsDict)
								{
									foreach (var detailPair in detailsDict)
									{
										...
										sb.Append($"                if (result._{SanitizeKey(pair.Key)}.Length > {i}{nullCheck}{detailNullCheck}) Debug.Log($\"{pair.Key}[{i}].{key}.{detailPair.Key}: {{result._{SanitizeKey(pair.Key)}[{i}]._{SanitizeKey(key)}._{SanitizeKey(detailPair.Key)}}}\"); // {detailPair.Value}\n");
									}
								}
								else
								{
									sb.Append($"                if (result._{SanitizeKey(pair.Key)}.Length > {i}{nullCheck}) Debug.Log($\"{pair.Key}[{i}].{key}: {{result._{SanitizeKey(pair.Key)}[{i}]._{SanitizeKey(key)}}}\"); // {value ?? "null"}\n");
								}
							}
						}
					}
					...
					{
						sb.Append($"                if (result._{SanitizeKey(pair.Key)}.Length > {i}) Debug.Log($\"{pair.Key}[{i}]: {{result._{SanitizeKey(pair.Key)}[{i}]}}\"); // {JsonTocs.ToJson(array[i])}\n");
					}
				}
			}
			else
			{
				...
				sb.Append($"                if (true{nullCheck}) Debug.Log($\"{pair.Key}: {{result._{SanitizeKey(pair.Key)}}}\"); // {pair.Value ?? "null"}\n");
			}
		}
				...
				if (data[i] is Dictionary<string, object> memberDict)
				{
					foreach (var key in allKeys)
					{
						if (memberDict.ContainsKey(key))
						{
							...
							if (value is object[] subArray)
							{
								sb.Append($"                if (result.Length > {i}{nullCheck}) Debug.Log($\"[{i}].{key}: {{string.Join(\", \", result[{i}]._{SanitizeKey(key)})}}\"); // {JsonTocs.ToJson(value)}\n");
							}
							else if (value is Dictionary<string, object> nestedDict)
							{
								foreach (var nestedPair in nestedDict)
								{
									...
									sb.Append($"                if (result.Length > {i}{nullCheck}{nestedNullCheck}) Debug.Log($\"[{i}].{key}.{nestedPair.Key}: {{result[{i}]._{SanitizeKey(key)}._{SanitizeKey(nestedPair.Key)}}}\"); // {nestedPair.Value}\n");
								}
							}
							else
							{
								sb.Append($"                if (result.Length > {i}{nullCheck}) Debug.Log($\"[{i}].{key}: {{result[{i}]._{SanitizeKey(key)}}}\"); // {value ?? "null"}\n");
							}
						}
					}
				}
