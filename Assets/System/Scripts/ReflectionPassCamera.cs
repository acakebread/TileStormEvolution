...
public class ReflectionPassCamera : CommandBufferSettings
{
	...
	private void InitializeReflectionCamera()
	{
		...
		// Add ReflectionRender component and initialize it
		var reflection = reflectionObj.AddComponent<ReflectionRender>();
		reflection.Initialize(mainCamera, planeNormal, offset, sceneCullingMask);
	...
	private void InitializeSceneCamera()
	{
		...
		// Initialize SceneRender with required fields
		sceneRender.Initialize(this, dimMesh, dimMaterial, dimColor);
		Debug.Log($"Initialized SceneRender: sceneCamera={sceneCamera}, dimColor={dimColor}, dimMaterial={dimMaterial}", this);
	...
	private class ReflectionRender : CommandBufferSettings
	{
		...
		public void Initialize(Camera referenceCamera, Vector3 planeNormal, float offset, LayerMask cullingMask)
		{
	...
	{
		public void Initialize(ReflectionPassCamera parent, Mesh dimMesh, Material dimMaterial, Color dimColor)
		{
			// Validate inputs
...
//		}
//		// Initialize SceneRender with required fields
//		sceneRender.Initialize(sceneCamera, reflectionCamera, dimMaterial, dimColor, this);
//		Debug.Log($"Initialized SceneRender: sceneCamera={sceneCamera}, reflectionCamera={reflectionCamera}, dimColor={dimColor}, dimMaterial={dimMaterial}", this);
//		Debug.Log("Initialized SceneCamera with SceneRender", this);
...
//		private ReflectionPassCamera parent;

//		public void SetParent(ReflectionPassCamera parentComponent)
//		{
//			parent = parentComponent;
//		}

//		public void Initialize(Camera sceneCamera, Camera reflectionCamera, Material dimMaterial, Color dimColor, ReflectionPassCamera parent)
//		{
//			this.sceneCamera = sceneCamera;
//			this.reflectionCamera = reflectionCamera;
//			this.dimMaterial = dimMaterial;
...
//					return;
//				}
//				this.dimMaterial = new Material(unlitShader);
//				this.dimMaterial.SetFloat("_Surface", 1.0f); // Transparent
//				this.dimMaterial.SetFloat("_SrcBlend", (float)BlendMode.SrcAlpha);
//				this.dimMaterial.SetFloat("_DstBlend", (float)BlendMode.OneMinusSrcAlpha);
//				this.dimMaterial.SetFloat("_SrcBlendAlpha", (float)BlendMode.SrcAlpha);
//				this.dimMaterial.SetFloat("_DstBlendAlpha", (float)BlendMode.OneMinusSrcAlpha);
//				this.dimMaterial.SetFloat("_ZWrite", 0.0f);
//				this.dimMaterial.SetFloat("_ZTest", (float)CompareFunction.LessEqual);
//				this.dimMaterial.SetFloat("_AlphaClip", 0.0f);
//				this.dimMaterial.EnableKeyword("_SURFACE_TYPE_TRANSPARENT");
//				this.dimMaterial.renderQueue = (int)RenderQueue.Transparent;
//			}

//			// Validate shader
//			if (this.dimMaterial.shader.name != "Custom/UnlitFixedColor" && this.dimMaterial.shader.name != "Universal Render Pipeline/Unlit")
//			{
//				Debug.LogWarning($"SceneRender on {gameObject.name}: Invalid shader {this.dimMaterial.shader.name}. Expected Custom/UnlitFixedColor or URP/Unlit.", this);
//			}
...
//					return;
//				}
//				commandBuffer.DrawMesh(dimMesh, Matrix4x4.identity, this.dimMaterial, 0, -1);
//			}, sceneCamera.name);
//		}
...
//			if (parent == null)
//			{
//				Debug.LogError($"SceneRender on {gameObject.name}: Parent ReflectionPassCamera is null", this);
//				return;
//			}

//			if (parent.planeNormal == Vector3.zero)
//			{
//				Debug.LogWarning($"SceneRender on {gameObject.name}: Invalid planeNormal (zero vector)", this);
//				return;
//			}
//			var n = parent.planeNormal.normalized;
//			var planePoint = n * parent.offset;
//			var plane = new Plane(n, planePoint);
