...
namespace ClassicTilestorm
{
	public static class MeshUtils
	{
		...
		private static void SubdivideMeshBSP(Mesh mesh, Vector3 divisionAxis, float minProj, float maxProj, float maxSegmentLength, Vector3 meshCenter, int depth, int maxDepth, List<Mesh> resultMeshes)
		{
			...
			Vector3 planePoint = meshCenter + divisionAxis * midProj;
			var splitResult = SplitMeshAlongPlane(mesh, divisionAxis, planePoint);

			if (Debug.isDebugBuild)
			{
				Debug.Log($"Split at depth {depth}, offset {midProj:F3}: top mesh {splitResult.topMesh.vertexCount} vertices, {splitResult.topMesh.triangles.Length / 3} triangles; bottom mesh {splitResult.bottomMesh.vertexCount} vertices, {splitResult.bottomMesh.triangles.Length / 3} triangles");
			}
		...
		public static SplitResult SplitMeshAlongPlane(Mesh mesh, Vector3 planeNormal, Vector3 planePoint)
		{
			...
			for (int i = 0; i < tris.Length; i += 3)
			{
				...
				ClipTriangle(tri, planeNormal, planePoint, topVerts, topTris, bottomVerts, bottomTris);
			}
			...
			List<VertexData> bottomVerts, List<int> bottomTris)
		{
			...
			ClipPolygonAgainstPlane(tri, planeNormal, planePoint, out topPoly, out bottomPoly);
			...
			out List<VertexData> topPoly, out List<VertexData> bottomPoly)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				float d1 = Vector3.Dot(curr.pos - planePoint, planeNormal);
				float d2 = Vector3.Dot(next.pos - planePoint, planeNormal);
...
//				ClipTriangle(tri, planeNormal, planePoint,
//					topVerts, topTris,
//					bottomVerts, bottomTris);
//			}
...
//		{
//			List<VertexData> topPoly, bottomPoly;
//			ClipPolygonAgainstPlane(tri, planeNormal, planePoint,
//				out topPoly, out bottomPoly);
