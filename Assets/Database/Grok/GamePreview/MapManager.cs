...
namespace GamePreviewNamespace
{
	public class MapManager : MonoBehaviour
	{
		...
		public bool CanMoveBetween(int fromTile, int toTile, int dirBit, int oppositeBit)
		{
			if (toTile < 0 || toTile >= Tiles?.Length) return false;
			int x = toTile % Width;
			int z = toTile / Width;
			if (x < 0 || x >= Width || z < 0 || z >= Height) return false;

			var fromDef = GetTileDefAt(fromTile);
			var toDef = GetTileDefAt(toTile);
		...
		private bool FindPathRecursive(int currentTile, int targetTile, int currentDirBit, List<int> currentPath, out List<int> resultPath)
		{
			...
			foreach (int dirBit in tryDirs)
			{
				...
				if (!CanMoveBetween(currentTile, nextTile, dirBit, dir.oppositeBit))
					continue;
...
//		public bool IsValidMove(int fromTile, int toTile, int dirBit, int oppositeBit)
//		{
//			if (toTile < 0 || toTile >= Tiles.Length) return false;
//			int x = toTile % Width;
//			int z = toTile / Width;
