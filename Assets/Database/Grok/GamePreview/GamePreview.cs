...
//		[SerializeField] private string geometryPath = "Geometry/fbx/";
//		[SerializeField] private string texturePath = "Textures/";
//		[SerializeField] private float tileMoveSpeed = 2f; // Eggbot movement speed
//		[SerializeField] private float pauseDuration = 1f; // Pause at waypoints

//		private Map currentMap;
//		private int width, height;
//		private GameObject mapRoot;
//		private GameObject eggbot;
//		private int[] tileMap; // Current tile positions (from mixed)
//		private List<int> waypoints; // nTile indices
//		private int currentWaypointIndex;
//		private bool isMoving;
//		private float moveTimer;
//		private float pauseTimer;
...
//		private bool isForwardBounce = true;
//		private Vector3 dragStartPos;
//		private bool isDragging;
//		private int dragTileIndex;
//		private int dragStride;
...
//			// Initialize tileMap from mixed
//			tileMap = currentMap.tiles?.nTileIndex?.unpacked_bytes;
//			if (tileMap == null || tileMap.Length != width * height)
//			{
//				Debug.LogError($"Invalid mixed data! mixed={currentMap.mixed != null}, nTileIndex={currentMap.mixed?.nTileIndex != null}, length={(tileMap != null ? tileMap.Length : -1)}, expected={width * height}");
//				return;
//			}
...
//					if (tileIndex < 0 || tileIndex >= currentMap.defs.Length)
//					{
//						Debug.LogWarning($"Invalid tileIndex={tileIndex} at ({x},{z}), defs.Length={currentMap.defs.Length}");
//						continue;
//					}

//					//TileDef tileDef = currentMap.defs[tileIndex];
//					//if (tileDef.szType == "tile_empty" || tileDef.szType == "tile_invisible")
//					//{
//					//	continue;
//					//}
...
//					Debug.Log($"Creating tile at ({x},{z}): tileIndex={tileIndex}, szType={tileDef.szType}, szGeom={tileDef.szGeom}");
...
//		{
//			int startTile = -1;
//			for (int i = 0; i < currentMap.defs.Length; i++)
//			{
//				int defIndex = tileMap[i];
//				string szType = currentMap.defs[defIndex].szType;
//				if ("tile_empty" == szType || "tile_invisible" == szType)
...
//				if (tileDef == null)
//				{
//					Debug.LogWarning($"TileDef not found for szType={szType}, szTheme={szTheme}) in map {currentMap.name}");
//					continue;
//				}
...
//				float t = Mathf.Clamp01(moveTimer);
//				int currentTile = waypoints[currentWaypointIndex];
//				int nextTile = waypoints[currentWaypointIndex + 1];
//				Vector3 startPos = new Vector3(currentTile % width, 1f, currentTile / width);
//				Vector3 endPos = new Vector3(nextTile % width, 1f, nextTile / width);
//				eggbot.transform.position = Vector3.Lerp(startPos, endPos, t);
...
//						if (currentTileDef != null && currentTileDef.bConsole)
//						{
//							isPuzzleBlocked = !CheckPathToNextWaypoint();
//							if (isPuzzleBlocked)
//							{
//								Debug.Log("Waiting at console for puzzle...");
//							}
...
//			if (isForwardBounce)
//			{
//				currentTile = waypoints[currentWaypointIndex];
//				nextTile = currentWaypointIndex + 1 < waypoints.Count ? waypoints[currentWaypointIndex + 1] : waypoints[currentWaypointIndex];
//			}
//			else
//			{
//				currentTile = waypoints[currentWaypointIndex];
//				nextTile = currentWaypointIndex - 1 >= 0 ? waypoints[currentWaypointIndex - 1] : waypoints[currentWaypointIndex];
//			}
...
//			if (eggbot != null)
//			{
//				Camera.main.transform.position = eggbot.transform.position + new Vector3(0, 5f, -5f);
//				Camera.main.transform.LookAt(eggbot.transform.position);
//			}
//		}
...
//				if (defIndex >= 0 && defIndex < currentMap.defs.Length)
//				{
//					return currentMap.defs[defIndex];
//				}
//			}
//			return null;
//		}

//		private bool CheckPathToNextWaypoint()
//		{
//			if (currentWaypointIndex + 1 >= waypoints.Count)
//			{
//				return false;
...
//			int currentX = currentTile % width;
//			int currentZ = currentTile / width;
//			int nextX = nextTile % width;
//			int nextZ = nextTile / width;

//			TileDef currentDef = GetTileDefAt(currentTile);
//			if (currentDef == null)
//			{
//				return false;
//			}

//			if (nextX == currentX + 1 && currentDef.bEast)
//			{
//				TileDef nextDef = GetTileDefAt(nextTile);
//				return nextDef != null && nextDef.bWest;
//			}
//			if (nextX == currentX - 1 && currentDef.bWest)
//			{
//				TileDef nextDef = GetTileDefAt(nextTile);
//				return nextDef != null && nextDef.bEast;
//			}
//			if (nextZ == currentZ + 1 && currentDef.bNorth)
//			{
//				TileDef nextDef = GetTileDefAt(nextTile);
//				return nextDef != null && nextDef.bSouth;
//			}
//			if (nextZ == currentZ - 1 && currentDef.bSouth)
//			{
//				TileDef nextDef = GetTileDefAt(nextTile);
//				return nextDef != null && nextDef.bNorth;
//			}
...
//					{
//						TileDef tileDef = GetTileDefAt(tileIndex);
//						if (tileDef != null && tileDef.bSlide)
//						{
//							isDragging = true;
//							dragStartPos = hitPos;
//							dragTileIndex = tileIndex;
...
//					{
//						dragStride = delta.x > 0 ? 1 : -1; // East/West
//					}
//					else
//					{
//						dragStride = delta.z > 0 ? width : -width; // North/South
//					}

//					SlideTiles(dragTileIndex, dragStride);
//				}
//			}
...
//		private void SlideTiles(int startTile, int stride)
//		{
//			List<int> tilesToMove = new List<int>();
//			bool canMove = true;
//			bool isRoll = false;
...
//			tilesToMove.Add(currentTile);
//			TileDef currentDef = GetTileDefAt(currentTile);
//			if (currentDef != null && currentDef.bRoll)
//			{
//				isRoll = true;
//			}
...
//				int nextX = nextTile % width;
//				int nextZ = nextTile / width;
//				if ((stride == 1 && currentX >= width - 1) ||
//					(stride == -1 && currentX <= 0) ||
//					(stride == width && currentZ >= height - 1) ||
//					(stride == -width && currentZ <= 0))
//				{
//					if (!isRoll)
//					{
//						canMove = false;
...
//				TileDef nextDef = GetTileDefAt(nextTile);
//				if (nextDef == null || (!nextDef.bSlide && !nextDef.bRoll))
//				{
//					canMove = false;
//					break;
//				}
...
//			{
//				int prevTile = currentTile - stride;
//				if (prevTile < 0 || prevTile >= tileMap.Length)
//				{
//					if (!isRoll)
//					{
//						canMove = false;
...
//				int currentX = currentTile % width;
//				int currentZ = currentTile / width;
//				int prevX = prevTile % width;
//				int prevZ = prevTile / width;
//				if ((stride == 1 && currentX <= 0) ||
//					(stride == -1 && currentX >= width - 1) ||
//					(stride == width && currentZ <= 0) ||
//					(stride == -width && currentZ >= height - 1))
//				{
//					if (!isRoll)
...
//				TileDef prevDef = GetTileDefAt(prevTile);
//				if (prevDef == null || (!prevDef.bSlide && !prevDef.bRoll))
//				{
//					canMove = false;
//					break;
//				}
...
//			if (isRoll)
//			{
//				int minTile = tilesToMove.Min();
//				int maxTile = tilesToMove.Max();
//				foreach (int tile in tilesToMove)
//				{
//					int newTile = tile + stride;
//					if (newTile < minTile || newTile > maxTile)
//					{
//						newTile = stride > 0 ? minTile : maxTile;
//					}
//					newTileMap[tile] = tileMap[newTile];
//				}
//			}
//			else if (canMove)
//			{
...
//					int newTile = tile + stride;
//					newTileMap[tile] = tileMap[newTile];
//					newTileMap[newTile] = tileMap[tile];
//				}
//			}
